...
class OptionSnapshotsTable:
    ...
    - theta: Option theta
    - vega: Option vega
    - active: Boolean - True if option is currently tradeable (has volume/oi)
    - new: Boolean - True if this is first time seeing this option
    ...
    def _ensure_table_exists(self) -> None:
        ...
        schema = pl.Schema({
            ...
            'theta': pl.Float64,
            'vega': pl.Float64,
            'active': pl.Boolean,  # True if tradeable (has volume/oi)
            'new': pl.Boolean,    # True if first time seeing this option
    ...
    def write_snapshots(self, contracts: List[OptionContract]) -> int:
        ...
        for contract in contracts:
            ...
            data.append({
                ...
                'active': is_active,
                'new': False,  # Will be updated during deduplication
                'yearmonth': yearmonth,
        ...
        if len(existing_filtered) > 0:
            ...
            # Set 'new' field: True if first time, False if existing
            new_snapshots = new_snapshots.with_columns(
                new=pl.col("timestamp_right").is_null()
            )

            # Drop the join column
            new_snapshots = new_snapshots.drop('timestamp_right')
        else:
            # All snapshots are new (first time seeing these contracts)
            new_snapshots = df_deduped.with_columns(
                new=pl.lit(True)
            )
